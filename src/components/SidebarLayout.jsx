...
const SidebarLayout = ({ children }) => {
  ...
  const links = [
    ...
    { to: "/masters", label: "Masters", icon: <Users className="w-5 h-5" />, hasArrow: true, submenu: masterLinks },
    { to: "/inventory", label: "Inventory", icon: <Package className="w-5 h-5" /> },
    { to: "/in-house-plating", label: "In House Plating", icon: <Layers className="w-5 h-5" /> },
    ...
    <div className="flex flex-col h-screen">
      <div className="flex flex-1 overflow-hidden">
        ...
        <aside
          className={`${
            isOpen ? "block" : "hidden"
          } md:block w-64 bg-white border-r border-gray-200 text-gray-800 fixed md:relative h-full z-20 overflow-y-auto scrollbar-hide`}
        >
                    ...
                    >
                      <div className={`flex items-center justify-between px-4 py-2 rounded-lg transition-colors ${
                        mastersOpen || location.pathname.includes("/masters")
                          ? "bg-white text-gray-900 font-semibold border-2 border-gray-400"
                        ...
                        <ChevronRight className={`w-5 h-5 text-gray-600 transition-transform ms-4 ${mastersOpen ? "rotate-90" : ""}`} />
                      </div>
                    </button>

                    {/* Submenu popover */}
                    {mastersOpen && (
                      <div className="fixed bg-white rounded-lg shadow-lg border border-gray-200 z-50 w-48" 
                        style={{
                          left: '19%',
                          top: '20%'
                        }}>
                        {link.submenu.map((sublink) => (
                          <Link
                            ...
                            className={`flex items-center px-4 py-3 transition-colors text-sm border-b border-b-gray-200 last:border-b-0 ${
                              location.pathname === sublink.to
                                ? "bg-white text-gray-900 font-semibold "
                                : "text-gray-700 hover:bg-gray-50"
                            }`}
                          >
                            {sublink.icon && <span className="mr-3 text-gray-600">{sublink.icon}</span>}
